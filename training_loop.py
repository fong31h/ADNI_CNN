...
if not path.exists(log_path):
    mkdir(log_path)
output_file = path.join(log_path, f'training_loop.py_{datetime}.log')
...
training_csv = path.join(training_loop_directory, f'ADNI/{input('Enter desired training loop data set (stored in ADNI/ folder): \n')}')

# print(training_csv)
# print(type(training_csv))

# Create the dataset
dataset = Import_classes.Rage_Scans(csv_file=training_csv,transform=transforms)
...
train_percent = float(input('Enter desired percentage of data to be used for training set (between 0 and 1): \n'))
validation_percent = float(input('Enter desired percentage of data to be used for validation set (between 0 and 1). Testing set will make up the remaining percentage: \n'))
    ...
    if avg_val_loss < best_val_loss - min_delta:
        ...
        for file in listdir('Models/'):
            if re.search(model_name, file):
                ...
                remove(file_path)
                break
        torch.save(resnet_50.state_dict(), f'Models/{model_name}_epoch_{epoch}.pth')  # Save the best model
    else:
